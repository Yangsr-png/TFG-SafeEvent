...
export default function PanicButton({ userId }: PanicButtonProps){
  ...
  // ------------------------------------------------------------------
  // NUEVO: Escuchar la orden de "Resolución" desde el Servidor
  // ------------------------------------------------------------------
  useEffect(() => {
    if (!socket) return;

    const handleResolvedByAdmin = () => {
      // 1. Apagamos la UI roja y detenemos la transmisión GPS
      cancelEmergency(); 
      // 2. Sobrescribimos el mensaje de reinicio con un micro-retraso para dar feedback
      setTimeout(() => {
        setStatusMsg('✅ Emergencia atendida y resuelta por el equipo médico.');
      }, 100);
    };

    socket.on('alert_resolved_by_admin', handleResolvedByAdmin);

    return () => {
      socket.off('alert_resolved_by_admin', handleResolvedByAdmin);
    };
  }, [socket]);
